	...
	if(tipo == "virtual"){
		...
		for(var i = 0; i < seleccionarDetalle.length; i++){
			...
			if($(seleccionarDetalle[i]).val() == ""){
				...
				swal({
					  title: "Debe seleccionar Cuanto y Servicio",
					  text: "",
					  type: "warning",
...
$("#cambiarDivisa").change(function(){
	...
	if($("#seleccionarPais").val() == ""){
		...
		$("#cambiarDivisa").after('<div class="alert alert-warning">No ha seleccionado la direccion de envío</div>');
	...
	$.ajax({
	    ...
	    success:function(respuesta){
	    	...
	    	var conversion = (respuesta["S/_"+divisa]).toFixed(2);
...
function pagarConPayu(){
	...
	if($("#seleccionarPais").val() == ""){
		...
		$(".formPayu").after('<div class="alert alert-warning">No ha seleccionado la direccion de envío</div>');
